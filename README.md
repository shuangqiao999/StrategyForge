...
4. 情报排序：量化模式用 `IntelSorter` LLM 分类实体 + `_apply_safety_net()` 确定性安全网（中英文关键词）；叙事模式用 `NarrativeSorter` 故事角色分类（含类型安全网 + 频率兜底），domain_prompts.json 按领域注入分类示例
5. 别名合并：Kuzu `merge_alias_nodes()` 重连关系边 + sorter 后刷新 intel_list 名称防止断层
...
每轮：
  ...
  7. 人格反思：事件驱动 + 环境感知 + 关系监控，三条件任一满足即触发（非固定轮次）
  8. 可选叙事 + 态势快照生成
```
...
- 关系网络变化：盟友/对手集合发生变化
- 长期无反思保护：超过 6 轮未触发任何反思

**执行逻辑**：
1. 收集 agent 的近期经历（量化：指标变化+因果反馈；叙事：亲历事件列表）
2. 调用轻量 LLM（temperature=0.3）生成一条行为准则（≤20 字）——量化模式基于指标变化与因果反馈，叙事模式要求准则**必须由具体亲历事件直接引出**，防幻觉
...
data/rule/        内置规则包 rules.json（8 域）+ domain_prompts.json（领域提示词配置）
