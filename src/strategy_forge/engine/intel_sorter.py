...
def _apply_safety_net(result: list[dict[str, Any]]) -> None:
    """保守安全网：对高置信的"非独立决策者"强制 include_in_simulation=false。

    仅在 FORGE_INTEL_SAFETY_NET 开启时生效；只降级、不新增/删除实体，避免误伤唯一代表。
    """
    import os
    if os.getenv("FORGE_INTEL_SAFETY_NET", "1") != "1":
        return

    def _demote(e: dict, note: str) -> None:
        if e["include_in_simulation"]:
    ...
    for e in result:
        ...
        if not name:
            continue
        if name in _DYAD_WORDS:
            _demote(e, "二元关系词")
        elif name.endswith(_UNIT_SUFFIX):
            _demote(e, "军队编制归上级")
        elif any(w in name for w in _DEPT_WORDS):
            _demote(e, "政府部门归上级国家")
        elif name.endswith(_TITLE_SUFFIX):
            _demote(e, "职务头衔非实体")
