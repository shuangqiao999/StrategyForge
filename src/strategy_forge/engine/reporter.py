...
1. "narrative": 完整推演报告的自然语言文本（800-2000字）。写作要求：
   ...
   - 本引擎定位战略推演，聚焦事物走向与趋势判断：用自然语言描述各方态势的方向性变化（如"某实体防御力量维持高位、财政持续承压"），准确刻画趋势即可
...
2. "risk_alerts": 风险预警列表（最多5条字符串）。每条格式：{风险标题} | {触发条件} | {受影响方}。示例："供应链断裂风险 | 连续多轮消耗无补充且库存逼近淘汰线 | 某实体"。禁止使用"可能/或将/存在风险"等模糊词前置——如果是风险，直接陈述事实。

3. "recommendations": 策略建议列表（最多5条字符串）。每条格式：{针对方}→{方向}→{效果}。示例："某实体→暂停扩张转向补充资源→预期2轮内恢复至安全水平"。每条不超过40字。不写"建议""应""需要"等虚词。
