...
1. **"narrative"**：完整推演报告（800-2000字）。
   ...
   - 开篇（150字内）：作为全文第一个 `###` 段落之前的引入段，直接点明全局核心矛盾与主要博弈轴线。**严禁以时间开头或描述时间背景**（如"2026年""今年""当前月份""第X季度""X月"）——推演不设定具体时间线，一律用"当前""现阶段"等无时间词。禁用"报告显示""推演表明""第X轮推演中"等废话。
...
def _strip_numeric_figures(text: str) -> str:
    ...
    # 保护时间数字：2026年 / 7月 / 15日 / 第3季度 / 10轮 / 5天 等
    # （时间单位前的数字是语境时间，不是指标数值，不应被替换成"显著"）
    text = _re.sub(r'\d+(?:\.\d+)?(?=\s*(?:年|月|日|天|周|季度|轮|期|时|分|秒|年代|世纪))', _hold, text)
